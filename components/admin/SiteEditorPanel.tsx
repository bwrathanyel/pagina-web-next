...
function HomeFields() {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="space-y-3 border-t border-ink/10 pt-5"><h3 className="font-display text-lg font-semibold text-ink">Cómo funciona</h3><Field path="home.process.eyebrow" label="Etiqueta" /><Field path="home.process.title" label="Título" /><Field path="home.process.description" label="Descripción" multiline />{content.home.process.steps.map((step, index) => <div key={index} className="space-y-2 rounded-xl bg-sand-2 p-3"><Field path={`home.process.steps.${index}.number`} label="Número" /><Field path={`home.process.steps.${index}.title`} label="Título" /><Field path={`home.process.steps.${index}.description`} label="Descripción" multiline /></div>)}</section>
