...
/** Proxy server-to-server hacia chat-demo-negocio, mismo patrón que
 * /api/perfil-instagram y /api/reescribir-descripcion: el navegador nunca
 * llama a la Edge Function directo porque su CORS apunta a producción.
 *
...
export async function POST(request: Request) {
  ...
  const url = process.env.CHAT_DEMO_NEGOCIO_URL;
  if (!url) {
  ...
  try {
    upstream = await fetch(url, {
      ...
      body: JSON.stringify({ nombre, tipo, descripcion, historial, mensaje }),
